...
CREATE TABLE `actuator_channels` (
  ...
  `current_level` DECIMAL(8,2) DEFAULT NULL COMMENT '定量程度：挡位/百分比/流量等',
...
CREATE TABLE `control_policies` (
  ...
  `effective_from` DATETIME(3) DEFAULT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `schedule_mode` VARCHAR(16) DEFAULT NULL COMMENT 'ONCE/DAILY/WEEKLY',
  `run_once_at` DATETIME(3) DEFAULT NULL COMMENT '单次执行时间',
  `time_of_day` TIME DEFAULT NULL COMMENT '每日/每周执行时刻',
  `weekdays_mask` TINYINT UNSIGNED DEFAULT NULL COMMENT '周一到周日 bitmask',
  `timezone` VARCHAR(64) NOT NULL DEFAULT 'Asia/Shanghai' COMMENT 'IANA 时区',
  `last_scheduled_for` DATETIME(3) DEFAULT NULL COMMENT '最近已处理计划点',
  `created_by` BIGINT UNSIGNED DEFAULT NULL,
  `published_by` BIGINT UNSIGNED DEFAULT NULL,
  `published_at` DATETIME(3) DEFAULT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
