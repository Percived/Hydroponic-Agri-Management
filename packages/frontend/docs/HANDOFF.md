...
- **`src/components/layout/AppSidebar.vue`**
  - 重构侧边栏菜单排序，遵循“核心生产优先 -> 监控运维 -> 自动化干预 -> 底层资产与系统”的业务逻辑。
  - 提升“批次管理”、“营养液管理”、“病虫害观察”至首要位置。
  - 新增“系统管理”一级子菜单，将底部的“用户管理”、“通知渠道”、“审计日志”聚合收纳，减少侧边栏垂直长度。
  - 为“策略控制”子菜单图标替换为 `<Operation />`，避免与“系统管理”的 `<Setting />` 图标冲突。
- **`src/router/index.ts`**
  - 同步调整 `routes` 数组顺序，使其与侧边栏的逻辑分组保持一致，增强代码可维护性。
